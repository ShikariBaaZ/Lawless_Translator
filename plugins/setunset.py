...
async def setlg(client,message):
    	    ...
    	    if lg_code:
    	    		...
    	    		try:
    	    			...
    	    			await message.reply_text("❗️ This language is Not available in My List \n Or Check Your spelling 😉",reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Check List 🔥" ,url="https://github.com/ShikariBaaZ/Lawless_Translator")]]))
    	    			return
    	    		set(user_id,lgcd)
    	    		await message.reply_text(f"Successfully set custom default language **{cd}**")
    	    else:
    	    		await message.reply_text(" Please use this command with an argument. \n **For Example:/set English**",reply_markup=InlineKeyboardMarkup([[	InlineKeyboardButton("Support 🙋‍♀️",url = "https://t.me/ShikariSupportNetwork")]]))
